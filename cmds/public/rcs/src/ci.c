...
#ifndef lint
 static char rcsid[]=
 "$Header: /usr/perihelion/Helios/cmds/gnu/rcs/src/RCS/ci.c,v 4.12 90/02/01 11:13:10 chris Exp $ Purdue CS";
#endif
/*******************************************************************
...
/* $Log:	ci.c,v $
 * Revision 4.12  90/02/01  11:13:10  chris
 * Fix argument to chmod and creat
 * 
 * Revision 4.11  90/01/02  11:20:14  chris
 * Helios port
 * 
...
char * argv[];
{
	...
	{ int xx;
        if ((xx = access(workfilename,R_OK))!=0) {
                error("working file %s not readable or nonexistent - %d",
                       workfilename,xx); 
                continue;
	} }
...
#define WWW (S_IWUSR|S_IWGRP|S_IWOTH)
	newRCSmode= (initflag|rcsinitflag?workstat.st_mode:RCSstat.st_mode)&~WWW;
...
{       register int c;
        ...
        if (quietflag || !ttystdin()) {
                ...
                VOID rcsfputs("checkin anyway? [ny](n): ",stderr); fflush(stderr);
                response=c=getchar();
 ...
 */
{
	...
	if (keepflag) {
		...
		VOID rcsfputs("reuse log message of previous file? [yn](y): ",stderr); fflush(stderr);
                cin=getchar();
		response=cin;
        ...
        if (ttystdin()) {
                VOID rcsfputs("enter log message:\n(terminate with ^D or single '.')\n>> ",stderr); fflush(stderr);
        ...
        for (;;) {
			...
			if ((tp==logmsg)||(*(tp-1)!='\n')) *tp++ = '\n'; /* append newline */
                ...
                if (tp>=logmsg+logsize-2) { /* overflow */
                        ...
                        VOID rcsfprintf(stderr,"log message too long. Maximum: %d\n",logsize);
                        VOID rcsfputs("reenter log message:\n>> ",stderr); fflush(stderr);
                        tp=logmsg; old1='\n'; old2=' ';
                        while (cin!='\n') cin=getchar(); /*skip line */
                        continue;
                }
                if (cin=='\n' && ttystdin()) VOID rcsfputs(">> ",stderr); fflush(stderr);
